...
class TestBedrockProvider:
    ...
    def test_init_without_credentials(self, mock_boto_client):
        ...
        BedrockProvider(region="us-west-2", model_id="test-model")
    ...
    def test_classify_email_success(
        self, mock_boto_client, test_email, test_labels, classification_prompt
    ):
        ...
        mock_body.read.return_value = (
            b'{"content": [{"text": "{\\"labels\\": [\\"AWS\\", \\"Finance\\"]}"}]}'
        )
    ...
    @patch("boto3.client")
    def test_classify_email_with_markdown_response(
        self, mock_boto_client, test_email, test_labels, classification_prompt
    ):
        ...
        response_text = '```json\\n{\\"labels\\": [\\"AWS\\", \\"Finance\\"]}\\n```'
        mock_body.read.return_value = (
            f'{{"content": [{{"text": "{response_text}"}}]}}'.encode()
        )
        mock_client.invoke_model.return_value = {"body": mock_body}
        mock_boto_client.return_value = mock_client
    ...
    @patch("boto3.client")
    def test_classify_email_client_error(
        self, mock_boto_client, test_email, test_labels, classification_prompt
    ):
    ...
    @patch("boto3.client")
    def test_classify_email_connection_error(
        self, mock_boto_client, test_email, test_labels, classification_prompt
    ):
    ...
    def test_classify_email_filters_invalid_labels(
        ...
        mock_body = Mock()
        mock_body.read.return_value = (
            b'{"content": [{"text": "{\\"labels\\": '
            b'[\\"AWS\\", \\"InvalidLabel\\", \\"Finance\\"]}"}]}'
        )
        mock_client.invoke_model.return_value = {"body": mock_body}
        mock_boto_client.return_value = mock_client
    ...
    @patch("boto3.client")
    def test_classify_email_empty_response(
        self, mock_boto_client, test_email, test_labels, classification_prompt
    ):
